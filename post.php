...
if (isset($_POST['delete'])) {
	...
	if (($config['country_flags'] && !$config['allow_no_country']) || ($config['country_flags'] && $config['allow_no_country'] && !isset($_POST['no_country']))) {
		...
		if ($country_code = geoip\geoip_country_code_by_addr_v6_forcedflag($gi, ipv4to6($_SERVER['REMOTE_ADDR']), $_SERVER['REMOTE_ADDR'])) {
			if (!in_array(strtolower($country_code), array('eu', 'ap', 'o1', 'a1', 'a2')))
				$post['body'] .= "\n<tinyboard flag>".strtolower($country_code)."</tinyboard>".
				"\n<tinyboard flag alt>".geoip\geoip_country_name_by_addr_v6_forcedflag($gi, ipv4to6($_SERVER['REMOTE_ADDR']), $_SERVER['REMOTE_ADDR'])."</tinyboard>";
		}
	}
